...
class MarimoComm:
    ...
    def _publish_msg(
        ...
        **keys: object,
        ...
        # Fast-path for common/expected message types, avoid repeating logic
    ...
    def flush(self) -> None:
        # Avoid repetitive repeated imports for every flush call
        # Move to staticmethod to import-once and minimize import overhead.
        if not hasattr(self, "_SendUIElementMessage"):
            from marimo._messaging.ops import SendUIElementMessage

            self._SendUIElementMessage = SendUIElementMessage
        SendUIElementMessage = self._SendUIElementMessage

        buffer = self._publish_message_buffer
        buf_pop = buffer.pop  # Method access optimization
        while buffer:
            item = buf_pop(0)
